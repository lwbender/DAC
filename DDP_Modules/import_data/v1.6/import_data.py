...
#~
qry = ''
with open('DDP_Modules/sql_queries/alpha_drm_query.sql', 'r') as f_sql:
            for rw in f_sql:
                rw = rw.rstrip('\n')
                rw = ' '.join(rw.split())
                #
...
def import_data(dosimeter_ids, SQL_cons = None, verbose = False, debug = False):
    ...
    data1 = process_qry(qrys[0], SQL_cons[0])
    data2 = process_qry(qrys[1], SQL_cons[0])
    configDB = get_configDB(qrys[2], SQL_cons[1])
    #~
    #combine data
    ...
    data = data.sort_values(by = ['dos_ID', 'mID'])
    #~
...
def get_qrys(dos_IDs):
    (RM_qry, DRM_qry) = get_alpha_qry(dos_IDs)
    configDB_qry = get_configDB_qry(dos_IDs)
    #~
    return (RM_qry, DRM_qry, configDB_qry)   
...
def get_alpha_qry(dos_IDs):
    ...
    for dos_ID in dos_IDs: dos_str += "(d.SerialNumber = '" + str(dos_ID) + "')" + " or "
    dos_str = dos_str[:-4]
    DRM_qry = qry2 + dos_str + ')'
    #~
    return (RM_qry, DRM_qry)   
