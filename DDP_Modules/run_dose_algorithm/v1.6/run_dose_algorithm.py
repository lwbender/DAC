...
def run_dose_algorithm(data, pCVs, prev_SDE, prev_DDE, prev_LDE, MRD, DAversion, verbose = False, debug = False):
    ...
    for PC in PCs: data['CCV_'+PC] = np.nan
    data['pSDE'] = np.nan
    data['pDDE'] = np.nan
    data['pLDE'] = np.nan
    ...
    for i, sstate in enumerate(SSs):
        ...
        #set CVs for ss
        for PC in PCs:
            data.loc[sample.index.tolist(), 'CCV_' + PC] = np.mean(sample[PC].iloc[:5])
        #~
        #cvs is CVs current - CVs prev    
        cvs = np.mean(sample[PCs].iloc[:5]) - np.mean(prev_samp[PCs].iloc[:5])
        for PC in PCs:
            data.loc[sample.index.tolist(), 'CV_' + PC] = np.mean(sample[PC].iloc[:5])- np.mean(prev_samp[PC].iloc[:5])
        #~
        ...
        if (da_output[0] < MRD) or (da_output[:4]<0).any() or pd.isnull(da_output[:3]).any(): #lower than MRD or negative energy
            ...
            for PC in PCs: 
                ...
                data.loc[sample.index.tolist(), 'CCV_' + PC] = np.mean(prev_samp[PC].iloc[:5])
        else:
        ...
        data.loc[sample.index.tolist(), 'pSDE'] = da_output[0]
        data.loc[sample.index.tolist(), 'pDDE'] = da_output[1]
        data.loc[sample.index.tolist(), 'pLDE'] = da_output[2]

    #~
    data['type']='dose_equiv'
    data[PCs]=PC_data-PC_data.iloc[0]
    #~
